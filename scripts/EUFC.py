...
def save_units_to_json(new_unit_codes, output_json):
    ...
    if os.path.exists(output_json):
        with open(output_json, 'r', encoding='utf-8') as f:
            ...
            existing_unit_codes = {entry["unitCode"] for entry in existing_data.get("unitCodes", [])}
    else:
    ...
    # Format the unit codes as a list of objects
    formatted_unit_codes = [{"unitCode": code} for code in sorted(updated_unit_codes)]

    # Save the updated unit codes back to the JSON file
    with open(output_json, 'w', encoding='utf-8') as f:
        json.dump({"unitCodes": formatted_unit_codes}, f, indent=4)
